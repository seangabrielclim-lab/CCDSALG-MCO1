...
int checkTurn(Point a, Point b, Point c){
	double area = (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x); // calculation of the cross product
...
/*
    ...
    f. Parameters: point a, b, c are individual points
*/
int grahamFast(Point points[], int n, Point convexHull[]){
	...
	quickSort(points, p0, 1, n-1); // fast sort algorithm call
